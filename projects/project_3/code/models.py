...
# ===================================================================
# 3. NEURAL NETWORK (FFNN) – from Project 2
# ===================================================================
class NeuralNetwork:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, epochs: int, batch_size: int,
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i in range(0, m, batch_size):
                ...
                if self.cost in ['cross_entropy', 'bce']:
                    yb_onehot = np.eye(self.layers[-1])[yb.ravel()] if self.cost == 'cross_entropy' else yb
                else:
                    yb_onehot = yb.reshape(-1, 1)
            ...
            if X_val is not None:
                ...
                if self.cost == 'mse':
                    ...
                    pred = (y_full > 0.5).astype(int)
                    metric = accuracy_score(y.ravel(), pred)
                history.append(metric)

            if verbose and epoch % max(1, epochs//10) == 0:
                print(f"Epoch {epoch:4d} | Val Loss: {history[-1]:.6f}")
    ...
    # NEW: Safe save/load methods
    def save(self, path: str):
        data = {
            'layers': self.layers,
            'activations': [
                'relu' if f is self._relu else
                'leaky_relu' if f is self._leaky_relu else
                'sigmoid' if f is self._sigmoid else
                'softmax' if f is self._softmax else
                'linear'
                for f in self.act_funcs
            ],
            'cost': self.cost,
            'seed': self.seed,
            'W': self.W,
...
# ===================================================================
# FINAL LINE: Allow direct import in part_c_implementation.py
# ===================================================================
__all__ = ['NeuralNetwork', 'Adam', 'RegressionModel', 'ClassificationModel']